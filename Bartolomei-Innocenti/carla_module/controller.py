...
class KeyboardControl(object):
    ...
    def _parse_vehicle_keys(self, world, keys, milliseconds):
        if world.useCruiseControl:
            if world.cruiseResult is not None:
                #frame, velocitaUscita, accelerazione, parseTime, inferenceTime, calibrationTime = world.cruiseResult
                #self._control.throttle = min(max(accelerazione * 0.1, 0), 1)
                #self._control.brake = min(max(-accelerazione * 0.1, 0), 1)
                self._control.throttle = min(max(world.pidController.throttle, 0), 1)
                self._control.brake = min(max(world.pidController.brake, 0), 1)
        else:
